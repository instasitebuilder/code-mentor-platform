...
import { Home, Info, LayoutDashboard, LogIn, HelpCircle, Users } from 'lucide-react';
import {
  Sidebar,
  SidebarContent,
  SidebarGroup,
...
import CompanyLogo from '@/assets/logo.jpg'; // Adjust the path as necessary

const navigationItems = [
  { title: 'Home', icon: Home, url: '/' },
  { title: 'Dashboard', icon: LayoutDashboard, url: '/dashboard' }, // Added Dashboard
  { title: 'About', icon: Info, url: '/about' },
  { title: 'Pricing', icon: LayoutDashboard, url: '/pricing' },
];
...
export function AppSidebar() {
  ...
  return (
    <Sidebar className="flex flex-col h-full">
      <SidebarContent className="flex flex-col flex-grow">
        ...
        <SidebarGroup>
          <SidebarGroupContent>
            <SidebarMenu>
              <SidebarMenuItem>
                <SidebarMenuButton onClick={() => navigate('/community')} className="flex items-center space-x-2">
                  <Users className="text-gray-500 dark:text-gray-300" />
                  <span>Community</span>
                </SidebarMenuButton>
              </SidebarMenuItem>
            </SidebarMenu>
      ...
      {/* Footer Actions (Shifted to Bottom) */}
      ...
      {/* Modal for predefined questions with answers */}
      {isModalOpen && (
